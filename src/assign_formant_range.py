...
""" Assign Common Voice speakers to high or low formant range,
		using SciKitLearn MinCovDet package and Mahalanobis distance.
	Read in 2 training files (given in Github repo).
	Apply over a pair of TSV 'test' formant files.
	Print to Standard Out.

	To run:
		python assign_formant_range.py {train_low_tsv} {train_high_tsv} {test_low_formant_tsv} {test_high_formant_tsv}
	ex:
		python src/assign_formant_range.py data/old_cv7/train_cv7_gender_low_mp.tsv data/old_cv7/train_cv7_gender_high_mp.tsv data/hausa_formants_low4.csv data/hausa_formants_high4.csv > data/cv8/formants/settings_highlow.tsv
	NOTE for Github: first 2 train files are given in Repo. Test 2 files can be found on OSF.
"""
...
def get_spkr_formants(formant_infile):
	...
	# 2 versions of code. Exactly same except for diff in loading encoding
	try:
...
def main():
	train_low_tsv = sys.argv[1]  # e.g. 'data/old_cv7/train_cv7_gender_low_mp.tsv'
	train_high_tsv = sys.argv[2]  # e.g. 'data/old_cv7/train_cv7_gender_high_mp.tsv'
	low_formant_infile = sys.argv[3]  # e.g. data/hausa_formants_low4.csv
	high_formant_infile = sys.argv[4]  # e.g. data/hausa_formants_high4.csv

	low_m_f1, low_m_f2, low_f_f1, low_f_f2 = process_gender_tsv(train_low_tsv)
	high_m_f1, high_m_f2, high_f_f1, high_f_f2 = process_gender_tsv(train_high_tsv)
